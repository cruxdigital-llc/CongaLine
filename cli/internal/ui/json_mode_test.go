...
func TestTextPromptJ_JSONMode(t *testing.T) {
	defer ResetJSONMode()
	_ = SetJSONMode(`{"name":"fromjson"}`)

	s, err := TextPromptJ("name", "Enter name")
	if err != nil || s != "fromjson" {
		t.Errorf("TextPromptJ = %q, %v; want %q, nil", s, err, "fromjson")
	}
}

func TestTextPromptJ_JSONMode_Missing(t *testing.T) {
	defer ResetJSONMode()
	_ = SetJSONMode(`{}`)

	_, err := TextPromptJ("name", "Enter name")
	if err == nil {
		t.Fatal("expected error for missing field in JSON mode")
	}
}

func TestTextPromptWithDefaultJ_JSONMode_Present(t *testing.T) {
	defer ResetJSONMode()
	_ = SetJSONMode(`{"image":"custom:latest"}`)

	s, err := TextPromptWithDefaultJ("image", "Docker image", "default:latest")
	if err != nil || s != "custom:latest" {
		t.Errorf("got %q, %v; want %q, nil", s, err, "custom:latest")
	}
}

func TestTextPromptWithDefaultJ_JSONMode_Missing(t *testing.T) {
	defer ResetJSONMode()
	_ = SetJSONMode(`{}`)

	s, err := TextPromptWithDefaultJ("image", "Docker image", "default:latest")
	if err != nil || s != "default:latest" {
		t.Errorf("got %q, %v; want %q, nil", s, err, "default:latest")
	}
}

func TestSecretPromptJ_JSONMode(t *testing.T) {
	defer ResetJSONMode()
	_ = SetJSONMode(`{"token":"secret123"}`)

	s, err := SecretPromptJ("token", "Enter token")
	if err != nil || s != "secret123" {
		t.Errorf("SecretPromptJ = %q, %v; want %q, nil", s, err, "secret123")
	}
}

func TestConfirmJ_JSONMode(t *testing.T) {
	defer ResetJSONMode()
	_ = SetJSONMode(`{}`)

	if !ConfirmJ("Continue?") {
		t.Error("ConfirmJ should return true in JSON mode")
	}
}

func TestConfirmJ_TextMode(t *testing.T) {
	defer ResetJSONMode()
	ResetJSONMode()

	// In text mode with no stdin, Confirm returns false
	// We can't easily test the interactive path here, just verify it doesn't panic
	// and returns false (no input available)
}
