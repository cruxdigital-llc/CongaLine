...
## Recent Changes
- 2026-03-26: Egress Domain Allowlisting — per-agent nginx proxy for SNI-based domain filtering across all three providers. Unified enforcement mechanism (no Squid/iptables split). Policy-driven via `conga-policy.yaml` egress section. Local: validate (warn) or enforce (proxy) modes. Remote/AWS: always enforce when domains defined. 2 new files, 6 modified, 11 unit tests. See `specs/2026-03-25_feature_egress-allowlist/`.
