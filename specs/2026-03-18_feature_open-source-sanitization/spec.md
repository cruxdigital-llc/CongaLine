...
```bash
# Before:
aws s3 cp s3://openclaw-terraform-state-167595588574/openclaw/bootstrap/bootstrap.sh /tmp/bootstrap.sh --region ${aws_region}
...
```bash
# Before:
aws s3 cp s3://openclaw-terraform-state-167595588574/openclaw/router/package.json ...
aws s3 cp s3://openclaw-terraform-state-167595588574/openclaw/router/src/index.js ...
...
**`cli/cmd/root.go`**:
- Line 69: `UA13HEGTS` → `UXXXXXXXXXX`
- Line 76: `C0ALL272SV8` → `CXXXXXXXXXX`
...
- `167595588574` → `<account_id>` (or describe the pattern)
- `UA13HEGTS` → `<member_id>`
- `C0ALL272SV8` → `<channel_id>`
- `aaronstone` → `<username>`
- `openclaw-aaron` → `openclaw-<member_id>`
...
Replace:
- `C0ALL272SV8` → `<channel_id>`
- `openclaw-terraform-state-167595588574` → `<project_name>-terraform-state-<account_id>`
