...
```bash
# Before:
aws s3 cp s3://conga-terraform-state-167595588574/conga/bootstrap/bootstrap.sh /tmp/bootstrap.sh --region ${aws_region}
...
```bash
# Before:
aws s3 cp s3://conga-terraform-state-167595588574/conga/router/package.json ...
aws s3 cp s3://conga-terraform-state-167595588574/conga/router/src/index.js ...
...
Replace:
- `C0ALL272SV8` → `<channel_id>`
- `conga-terraform-state-167595588574` → `<project_name>-terraform-state-<account_id>`
