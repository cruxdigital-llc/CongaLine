...
- Store in shell variables (never write to disk):
  ...
  ANTHROPIC_API_KEY=$(aws secretsmanager get-secret-value --secret-id conga/aaron/anthropic-api-key --query SecretString --output text)
  TRELLO_API_KEY=$(aws secretsmanager get-secret-value --secret-id conga/aaron/trello-api-key --query SecretString --output text)
  TRELLO_TOKEN=$(aws secretsmanager get-secret-value --secret-id conga/aaron/trello-token --query SecretString --output text)
  ```
...
- Create directory `/opt/conga/data/aaron/` for workspace and memory
- Owned by conga user (uid 1000)
- This lives on the root EBS volume (encrypted via KMS)

### 2f. Create Docker Network
- Isolated bridge network for Aaron: `conga-aaron`
...
- Unit configuration:
  ...
  ExecStart=/usr/bin/docker run \
    ...
    -v /opt/conga/data/aaron:/home/node/.openclaw/data:rw \
    -e ANTHROPIC_API_KEY \
    -e SLACK_BOT_TOKEN \
    -e SLACK_APP_TOKEN \
    -e TRELLO_API_KEY \
