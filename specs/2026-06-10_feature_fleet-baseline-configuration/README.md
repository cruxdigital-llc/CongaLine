...
- **Remaining (in order)**: **P7** `conga agent show-config` (CLI+JSON+MCP) → **P8** `config-taxonomy.md` docs → **P9** tests (incl. T4.5 Go-provider deploy tests) + live verify. Then review + verify gates → merge → `terraform-provider-conga` release → deployed-path verification.
