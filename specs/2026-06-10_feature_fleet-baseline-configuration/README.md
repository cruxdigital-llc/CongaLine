...
- **Done + committed + green (this session)**: **T4.4** AWS bash fresh-deploy path — `deploy-agents.sh` deploys fleet-custom.json + agent-managed-custom.json from S3-synced sources (root:root 0444, openclaw-gated); 3-element `$include` jq at all 3 config-write sites (add-user/add-team/user-data.tftpl). Render + content tests.
- **Remaining (in order)**: **P5** extend reserved-key guard + hashing to all managed layers (local/remote Go + AWS `check-config-integrity.sh`) → **P6** pre-deploy validation (fleet blast-radius fail-closed) + egress-gap warnings → **P7** `conga agent show-config` (CLI+JSON+MCP) → **P8** `config-taxonomy.md` docs → **P9** tests (incl. T4.5 Go-provider deploy tests) + live verify. Then review + verify gates → merge → `terraform-provider-conga` release → deployed-path verification.
