...
Wait — `%N` is the systemd unit name (e.g. `conga-aaron`), but we need the agent name (e.g. `aaron`). Use a variable substitution:
...
```
ExecStartPre=/bin/bash -c 'aws s3 sync s3://conga-terraform-state-123456/conga/behavior/ /opt/conga/behavior/ --region us-east-1 2>/dev/null || true; /opt/conga/bin/deploy-behavior.sh aaron $(cat /opt/conga/config/aaron.type 2>/dev/null || echo user)'
```
