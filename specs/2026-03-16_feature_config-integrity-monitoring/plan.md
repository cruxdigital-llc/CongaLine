...
- **Simplest approach**: The systemd unit already sends container stdout/stderr to the journal. Configure CloudWatch agent to read from the journal for the `conga-aaron` unit.
...
CURRENT_HASH=$(sha256sum /opt/conga/data/aaron/openclaw.json | cut -d' ' -f1)
if [ "$EXPECTED_HASH" != "$CURRENT_HASH" ]; then
  echo "CONFIG_INTEGRITY_VIOLATION user=aaron expected=$EXPECTED_HASH actual=$CURRENT_HASH"
  logger -t conga-integrity "CONFIG_INTEGRITY_VIOLATION user=aaron"
fi
```
...
```json
{
  "logs": {
    "logs_collected": {
      "journald": {
        "unit": ["conga-aaron", "conga-config-check"],
