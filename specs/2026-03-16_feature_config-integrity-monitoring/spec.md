...
if [ "$EXPECTED_HASH" != "$CURRENT_HASH" ]; then
  echo "CONFIG_INTEGRITY_VIOLATION user=aaron expected=$EXPECTED_HASH actual=$CURRENT_HASH" | systemd-cat -t openclaw-integrity -p warning
else
  echo "Config integrity OK user=aaron hash=$CURRENT_HASH" | systemd-cat -t openclaw-integrity
fi
CHECKSCRIPT
...
5. Tamper test (optional):
   - Modify `/opt/openclaw/data/aaron/openclaw.json` slightly
